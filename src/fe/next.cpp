...
cb::fe::Token &cb::fe::Parser::Next()
{
    ...
    if (c < 0x00) return m_Token = Token(m_Line, false);
...
bool cb::fe::Parser::AtEOF() const { return m_Token.Type == TokenType_EOF; }

bool cb::fe::Parser::At(const std::string &value) const { return m_Token.Value == value; }
...
cb::fe::Token cb::fe::Parser::Expect(TokenType type)
{
    if (!At(type))
        throw std::runtime_error("unexpected type");
    Token tk = m_Token;
